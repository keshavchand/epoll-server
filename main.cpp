...
int createServer(int port = 8080, int maxConn = SOMAXCONN) {
	...
	int enable = 1;
	int err = setsockopt(serverFd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(enable));
	if (err < 0) {
		return err;
	}
...
void AcceptClient(int serverFd, IoMux& ioMux, ClientMap& clientMap) {
	...
	int clientSock = accept4(serverFd, (sockaddr*) &client, &clientSockLen, SOCK_NONBLOCK);
	if (clientSock == -1) {
...
void IncommingData(IoMux& ioMux, ClientMap& clientMap, epoll_event event) {
	...
	while (i - offset > 0) {
		int shouldWrite = 0;
		offset += ExtendMessage(&cw->inMessage, data + offset, i - offset, &shouldWrite);

		if (!shouldWrite) continue;

		// TODO: Check is the message is full
		for (auto c = clientMap.begin(); c != clientMap.end(); c++){
			if (c->first == fd) continue;
			AddMessage(c->second, cw->inMessage);
		}

		cw->inMessage = Message{size : 0, written : 0};
	}
}
